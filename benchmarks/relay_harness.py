...
class RelayBenchmarkHarness:
    ...
    def _install_upstream_route(self) -> None:
        ...
        async def benchmark_endpoint(request: Request):
            ...
            return Response(
                self.response_factory(body), media_type="application/octet-stream"
            )
    ...
    async def __aenter__(self) -> "RelayBenchmarkHarness":
        ...
        self.relay_app.include_router(
            router(
                self.settings,
                ReplayCache(),
                Forwarder(self.upstream_client, self.settings.max_response_body_bytes),
            )
        )
        self.carrier_client = httpx.AsyncClient(
            transport=httpx.ASGITransport(app=self.relay_app),
