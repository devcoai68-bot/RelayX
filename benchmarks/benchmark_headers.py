...
def _headers(
    count: int, size: int, duplicate: bool = False, cookie: bool = False
) -> tuple[tuple[str, str], ...]:
    if cookie:
        return (("cookie", "x" * size),) + tuple(
            (f"x-header-{i}", "v") for i in range(max(0, count - 1))
        )
    if duplicate:
    ...
    for count, size, duplicate, cookie, label in scenarios:
        ...
        rows.append(
            (
                ...
                format_bytes(size),
                str(filtered_count),
                f"{iterations / seconds:.2f}",
                f"{seconds / iterations * 1000:.4f}",
            )
        )
...
def main() -> None:
    args = parse_common_args(
        "Benchmark RelayX HTTP header filtering", default_iterations=50_000
    )
