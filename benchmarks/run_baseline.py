...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the RelayX performance baseline suite and export JSON/Markdown reports."
    )
    parser.add_argument(
        "--iterations",
        type=int,
        default=5,
        help="Measured iterations for most benchmarks.",
    )
    parser.add_argument(
        "--warmup", type=int, default=1, help="Warmup iterations for most benchmarks."
    )
    parser.add_argument(
        "--throughput-iterations",
        type=int,
        default=None,
        help="Measured requests for throughput; defaults to --iterations.",
    )
    parser.add_argument(
        "--replay-iterations",
        type=int,
        default=None,
        help="Measured replay cache operations; defaults to --iterations.",
    )
    parser.add_argument(
        "--memory-iterations",
        type=int,
        default=1,
        help="Measured iterations for allocation-heavy memory benchmark.",
    )
    parser.add_argument(
        "--output-dir",
        default="benchmarks/results",
        help="Directory for machine-readable JSON results.",
    )
    parser.add_argument(
        "--markdown-output",
        default="docs/performance-baseline.md",
        help="Markdown baseline report path.",
    )
    parser.add_argument(
        "--date",
        default=date.today().isoformat(),
        help="Date stamp for JSON filename in YYYY-MM-DD format.",
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="Use full Phase 5 payload and replay-cache sizes. Defaults to quick mode for routine reproducible runs.",
    )
...
def _run_all(args: argparse.Namespace) -> tuple[BenchmarkResult, ...]:
    ...
    return (
        ...
        run_async(
            benchmark_throughput.benchmark_async(throughput_iterations, args.warmup)
        ),
        ...
        run_async(
            benchmark_large_payloads.benchmark_async(args.iterations, args.warmup)
        ),
    )
...
def _benchmark_size_mode(full: bool) -> Iterator[None]:
    ...
    original_values = {
        benchmark_latency: (
            "PAYLOAD_SIZES",
            benchmark_latency.PAYLOAD_SIZES,
            QUICK_PAYLOAD_SIZES,
        ),
        benchmark_compression: (
            "PAYLOAD_SIZES",
            benchmark_compression.PAYLOAD_SIZES,
            QUICK_PAYLOAD_SIZES,
        ),
        benchmark_crypto: (
            "PAYLOAD_SIZES",
            benchmark_crypto.PAYLOAD_SIZES,
            QUICK_PAYLOAD_SIZES,
        ),
        benchmark_large_payloads: (
            "LARGE_PAYLOAD_SIZES",
            benchmark_large_payloads.LARGE_PAYLOAD_SIZES,
            QUICK_LARGE_PAYLOAD_SIZES,
        ),
        benchmark_memory: (
            "LARGE_PAYLOAD_SIZES",
            benchmark_memory.LARGE_PAYLOAD_SIZES,
            QUICK_LARGE_PAYLOAD_SIZES,
        ),
        benchmark_replay: (
            "REPLAY_CACHE_SIZES",
            benchmark_replay.REPLAY_CACHE_SIZES,
            QUICK_REPLAY_CACHE_SIZES,
        ),
    }
    try:
