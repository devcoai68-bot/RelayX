...
def benchmark(iterations: int, warmup: int) -> BenchmarkResult:
    ...
    for size in LARGE_PAYLOAD_SIZES:
        ...
        rows.append(
            (
                format_bytes(size),
                format_bytes(max(peak_values)),
                str(max(allocation_counts)),
                format_bytes(max(peak_values) - size),
            )
        )
...
def main() -> None:
    args = parse_common_args(
        "Benchmark RelayX memory allocation behavior", default_iterations=3
    )
