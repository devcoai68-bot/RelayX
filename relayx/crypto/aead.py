...
from relayx.constants import AAD_SIZE, AEAD_NONCE_SIZE, ALLOWED_FLAGS, FLAG_COMPRESSED, HEADER_SIZE, MAGIC, NONCE_ID_SIZE, RESERVED, VERSION
...
def seal(plaintext: bytes, key: bytes, type_id: int, compressed: bool, timestamp_ms: int | None = None) -> bytes:
...
def open_packet(packet: bytes, key: bytes, max_ciphertext_bytes: int) -> OpenedPacket:
    ...
    aad = packet[:AAD_SIZE]
    magic, version, type_id, flags, reserved, timestamp_ms, nonce_id, aead_nonce = _parse_aad(aad)
    if magic != MAGIC or version != VERSION or type_id not in {1,2,3} or flags & ~ALLOWED_FLAGS or reserved != RESERVED:
    ...
    return OpenedPacket(type_id, bool(flags & FLAG_COMPRESSED), timestamp_ms, nonce_id, plaintext)

def _aad(type_id: int, flags: int, timestamp_ms: int, nonce_id: bytes, aead_nonce: bytes) -> bytes:
    if len(nonce_id) != NONCE_ID_SIZE or len(aead_nonce) != AEAD_NONCE_SIZE:
        raise ProtocolError("invalid nonce size")
    return struct.pack("!4sBBBBQ16s12s", MAGIC, VERSION, type_id, flags, RESERVED, timestamp_ms, nonce_id, aead_nonce)
