...
class LocalProxy:
    ...
    async def handle(
        ...
        try:
            http_request = await read_request(
                reader,
            ...
            request = RelayRequest(
                ...
                http_request.path,
                http_request.query,
                filter_forward_headers(http_request.headers),
                http_request.body,
            )
            response = await self.relay_client.send(request)
        except Exception:
            response = RelayError(
                None, "bad_request", "Request could not be processed", False
            )
    ...
    async def serve(self) -> asyncio.AbstractServer:
        return await asyncio.start_server(
            self.handle, self.settings.proxy_host, self.settings.proxy_port
        )
