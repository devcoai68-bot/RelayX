...
class RelayClient:
    def __init__(
        self, settings: RelaySettings, client: httpx.AsyncClient | None = None
    ) -> None:
        self.settings = settings
        self.key = settings.encryption_key_bytes
        self.client = client or httpx.AsyncClient(
            http1=True, http2=False, timeout=settings.timeout_seconds
        )
    ...
    async def send(self, message: RelayRequest) -> RelayResponse | RelayError:
        packet = encode_message(
            ...
            compression_threshold=self.settings.compression_threshold_bytes,
        )
        ...
        reply = decode_message(
            ...
            max_ciphertext_bytes=self.settings.max_carrier_body_bytes,
            max_decompressed_bytes=self.settings.max_decompressed_bytes,
            max_request_body_bytes=self.settings.max_request_body_bytes,
            max_response_body_bytes=self.settings.max_response_body_bytes,
        )
