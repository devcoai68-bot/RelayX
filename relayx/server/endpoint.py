...
from relayx.errors import (
    PacketExpiredError,
    PacketFromFutureError,
    ReplayCacheFullError,
    ReplayDetectedError,
...
def _error_from_exception(exc: Exception, request_id: str | None = None) -> RelayError:
    ...
    if isinstance(exc, RequestTooLargeError):
        return RelayError(
            request_id, "request_too_large", "Request body is too large", False
        )
    if isinstance(exc, ResponseTooLargeError):
        return RelayError(
            request_id, "response_too_large", "Response body is too large", False
        )
    return RelayError(
        request_id, "invalid_packet", "Relay packet could not be processed", False
    )
...
def router(
    settings: RelaySettings, replay_cache: ReplayCache, forwarder: Forwarder
) -> APIRouter:
    api = APIRouter()
    ...
    async def relay(
        ...
        logger.info(
            "relay_request_received", extra={"relayx": {"path": request.url.path}}
        )
        ...
        try:
            decoded = decode_message(
                ...
                max_ciphertext_bytes=settings.max_carrier_body_bytes,
            ...
            logger.info(
                ...
                extra={"relayx": {"request_id": message.request_id}},
            )
