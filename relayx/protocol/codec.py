...
def loads(
    data: bytes, expected_type: int | None = None
) -> RelayRequest | RelayResponse | RelayError:
    try:
    ...
    if not isinstance(raw, dict):
        raise SerializationError("message must be a map")
...
def _require_optional_str(raw: dict[str, Any], key: str) -> str | None:
    ...
    if value is not None and not isinstance(value, str):
        raise SerializationError(f"{key} must be a string or nil")
    return value
...
def _require_optional_int(raw: dict[str, Any], key: str) -> int | None:
    ...
    if value is not None and type(value) is not int:
        raise SerializationError(f"{key} must be an integer or nil")
    return value
...
def _require_body(raw: dict[str, Any]) -> bytes:
    ...
    if not isinstance(value, bytes):
        raise SerializationError("body must be binary")
    return value


def _headers(value: Any) -> HeaderList:
    if not isinstance(value, list):
        raise SerializationError("headers must be an array")
    out = []
    for item in value:
        if not (
...
def _request(raw: dict[str, Any]) -> RelayRequest:
    ...
    return RelayRequest(
        _require_str(raw, "request_id"),
        _require_str(raw, "method"),
        _require_str(raw, "scheme"),
        _require_str(raw, "host"),
        _require_optional_int(raw, "port"),
        _require_str(raw, "path"),
        _require_str(raw, "query"),
...
def _response(raw: dict[str, Any]) -> RelayResponse:
    ...
    return RelayResponse(
        _require_str(raw, "request_id"),
        _require_int(raw, "status_code"),
        _require_optional_str(raw, "reason_phrase"),
        _headers(raw["headers"]),
        _require_body(raw),
    )
...
def _error(raw: dict[str, Any]) -> RelayError:
    ...
    if request_id is not None and not isinstance(request_id, str):
        raise SerializationError("request_id must be a string or nil")
    return RelayError(
        request_id,
        _require_str(raw, "error_code"),
        _require_str(raw, "message"),
        _require_bool(raw, "retryable"),
    )
