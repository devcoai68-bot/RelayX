...
def validate_http_token(value: str, field: str) -> None:
    if (
        ...
        or not value
        or _has_ctl(value)
        or any(ch in _TOKEN_SEPARATORS for ch in value)
    ):
...
class RelayRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.type != "request":
            raise ProtocolError("invalid request type")
        validate_http_token(self.method, "method")
        if self.method.upper() == "CONNECT":
            raise ProtocolError("CONNECT is unsupported")
        if self.scheme not in {"http", "https"}:
            raise ProtocolError("invalid scheme")
        if (
        ...
        if not isinstance(self.body, bytes):
            raise ProtocolError("body must be bytes")
...
class RelayResponse:
    ...
    def __post_init__(self) -> None:
        ...
        if self.reason_phrase is not None and (
            not isinstance(self.reason_phrase, str)
            or "\r" in self.reason_phrase
            or "\n" in self.reason_phrase
        ):
...
class RelayError:
    ...
    def __post_init__(self) -> None:
        ...
        if self.version != VERSION or self.type != "error":
            raise ProtocolError("invalid error metadata")
...
def validate_common(version: int, type_name: str, request_id: str) -> None:
    ...
    if type_name not in {"request", "response", "error"}:
