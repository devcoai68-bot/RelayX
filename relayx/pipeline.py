...
from relayx.constants import (
    ...
    DEFAULT_MAX_CARRIER_BODY_BYTES,
...
def encode_message(
    ...
    *,
    ...
    compression_threshold: int = DEFAULT_COMPRESSION_THRESHOLD_BYTES,
) -> bytes:
    serialized = codec.dumps(message)
    payload, compressed = maybe_compress(
        serialized, compression_enabled, compression_threshold
    )
...
def decode_message(
    ...
    *,
    ...
    max_ciphertext_bytes: int = DEFAULT_MAX_CARRIER_BODY_BYTES,
    ...
    serialized = maybe_decompress(
        opened.plaintext, opened.compressed, max_decompressed_bytes
    )
    ...
    if (
        isinstance(message, RelayResponse)
        and len(message.body) > max_response_body_bytes
    ):
