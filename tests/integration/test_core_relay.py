...
async def test_client_server_upstream_roundtrip():
    ...
    async def echo(request: Request):
        ...
        return Response(
            body, media_type="application/octet-stream", headers={"x-upstream": "yes"}
        )
    ...
    async with httpx.AsyncClient(
        ...
        relay_app.include_router(
            router(server_settings, ReplayCache(), Forwarder(upstream_client, 1024))
        )
        async with httpx.AsyncClient(
            transport=relay_transport, base_url="http://relay.test"
        ) as carrier_client:
            client_settings = RelaySettings(
                auth_token="auth",
                encryption_key=KEY,
